...
use ebpf_prog::{types::pkt_action, DpitSkel};
use libbpf_rs::{
...
pub trait DpitSkelLib {
    fn sni_lpm_add_entry(&self, domain: &str, action: pkt_action) -> Result<()>;
}
...
impl<'obj> DpitSkelLib for DpitSkel<'obj> {
    ...
    fn sni_lpm_add_entry(&self, domain: &str, action: pkt_action) -> Result<()> {
        ...
        let value = &action as *const ebpf_prog::types::pkt_action as *const u8;
        let size = mem::size_of::<ebpf_prog::types::pkt_action>();
